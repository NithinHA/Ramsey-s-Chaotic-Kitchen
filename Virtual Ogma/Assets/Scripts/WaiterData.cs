...
public class WaiterData : MonoBehaviour
{
	...
	public Dictionary<Transform, Vector3> waiter_interactable_positions = new Dictionary<Transform, Vector3>();     // A dictionary formed out of above info

	void Awake()
    {
		for (int i = 0; i < waiter_interactable.Count; i++)                                 // initialize waiter_interactable_positoins dictionary
		{
			waiter_interactable_positions.Add(waiter_interactable[i], waiter_positions[i].position);
		}
	}
